...
try:
    from transformers import (
        ...
        Qwen2_5_VLForConditionalGeneration,  # <-- FIX for your NameError
...
# (Optional) OpenEMMA descriptions (kept, but not essential)
...
def main():
    ...
    for index in tqdm.tqdm(range(len(segments_json[id]))):
        ...
        image_history = []
        for segment in segments_json[id][:index+1]:
        ...
        image_history = image_history[-IMAGE_BUFFER_SIZE:] #only use up to last 10 frames
